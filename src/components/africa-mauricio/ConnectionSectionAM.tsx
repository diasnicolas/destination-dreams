...
import { Plane, Camera, Utensils, Waves } from "lucide-react";

const credentials = [
  { icon: Camera, text: "🦁 Big Five — Leão, Leopardo, Rinoceronte, Elefante, Búfalo" },
  { icon: Plane, text: "🚁 Voo privado — Johannesburg → Kruger (não van de 7h)" },
  { icon: Camera, text: "🌅 3 safáris inclusos — Manhã e tarde" },
  { icon: Waves, text: "🏊 Piscina em cada quarto — Conforto no Kruger" },
];

export const ConnectionSectionAM = () => {
  return (
    <section className="py-20 md:py-32 w-full bg-gradient-to-b from-primary/10 via-background to-primary/10" id="solution">
      <div className="w-full px-4 md:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-bold mb-6">Apresento: África do Sul & Ilhas Maurício</h2>
          <p className="text-xl text-primary font-semibold">Safári, Vinícolas e Praias Paradisíacas</p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.4 }} className="max-w-5xl mx-auto">
          ...
          <div className="flex flex-wrap justify-center gap-3 mt-8">
            {["🦛 Jessica a Hipopótamo", "✈️ Voo direto Kruger → Cape Town", "🏔️ Table Mountain", "🍷 Vinícolas Stellenbosch", "🏝️ Maurício All Inclusive"].map((h, i) => (
